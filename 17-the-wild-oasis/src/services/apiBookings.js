...
export const getBookings = async ({ filter, sortBy, page }) => {
    ...
    if (sortBy) {
        ...
        query = query.order(sortBy.field, options);
    }
...
export const getStaysTodayActivity = async () => {
    const unconfirmed = `and(status.eq.unconfirmed,startDate.eq.${getToday()})`;
    const checkedIn = `and(status.eq.checked-in,endDate.eq.${getToday()})`;
    const { data, error } = await supabase
        .from('bookings')
        .select('*, guests(fullName, nationality, countryFlag)')
        .or(`${unconfirmed},${checkedIn}`)
        .order('created_at');
